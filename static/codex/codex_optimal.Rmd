...
author: "Mark Webster"
date: |
  | Page last updated 2020-09-05
  | Data last updated 2020-10-05
self_contained: no
lib_dir: libs
---
...
```{r, "extract monodecks"}
mononames <- sort(paste0("Mono", setdiff(starters$starter[!endsWith(starters$starter, "F20")], "Neutral")))
```
...
knitr::kable(multinash_comps[is.element(Name, starters$starter),
...
Black and Necromancy are dominant, as expected. Bashing rates unexpectedly high, despite being considered weak. This is likely to be due to it not being used often in tournament play: this results in high uncertainty for its strength in the model, so occasionally a simulated sample considers it to be a strong component, rating highly in the sample's Nash equilibrium.
...
Black is the most-likely-best counter to half the monocolour decks. The exceptions are Red (White), Purple (Green), and White (Purple).
