...
author: "Mark Webster"
date: |
  | Page last updated 2020-10-30
  | Data last updated 2022-01-14
self_contained: no
lib_dir: libs
---
...
one_year_ago <- as.Date("2021-01-14")
```
...
latest_play_dates <- rbind(
  matches[!is.na(end)][, .(last = max(end, na.rm = TRUE)), by = "player1"][, .(player = player1, last = last)],
  matches[!is.na(end)][, .(last = max(end, na.rm = TRUE)), by = "player2"][, .(player = player2, last = last)]
...
vs_monos_both <- vs_monos[order(P1, P2)
                          ][, .(P1, P2,
                                prob_matchup = prob_matchup/2 + vs_monos[, .(P1 = P2, P2 = P1,
                                                                             prob_matchup = 1 - prob_matchup)
                                                                         ][order(P1, P2)
                                                                           ][, prob_matchup/2])
...
vs_monos_both_print <- vs_monos_both[, .(`P1 win probability` = mean(prob_matchup)), 
                                     by = c("P1", "P2")
                                       ...
                                       .(P1, P2, `P1 win probability`,
                                         matchup = paste(format(round(`P1 win probability`, 2)*10, nsmall = 1),
                                                         ...
                                                         sep = "-"),
                                         fairness = 1 - 2*abs(`P1 win probability` - 1/2))]
formatRound(datatable(vs_monos_both_print, rownames = FALSE, filter = "top",
                      options = list(dom = "ftip")), 
            c(3, 5), c(3, 2))
```
...
vs_monos_P1 <- vs_monos[order(P1, P2)
                        ][, .(P1, P2,
                              prob_matchup = prob_matchup/2 + vs_monos[, .(P1 = P2, P2 = P1,
                                                                           prob_matchup)
                                                                       ][order(P1, P2)
...
vs_monos_P1_print <- vs_monos_P1[, .(`P1 win probability` = mean(prob_matchup)), 
                                 by = c("P1", "P2")
                                 ][as.character(P1) <= as.character(P2),
                                   .(P1, P2, `P1 win probability`,
                                     matchup = paste(format(round(`P1 win probability`, 2)*10, nsmall = 1),
                                                     ...
                                                     sep = "-"),
                                     fairness = 1 - 2*abs(`P1 win probability` - 1/2))]
formatRound(datatable(vs_monos_P1_print, rownames = FALSE, filter = "top",
                      options = list(dom = "ftip")), 
            c(3, 5), c(3, 2))
```
