...
```{r, "current matchups"}
current_players <- entries[tournament == current, player]
...
Decks were not fixed for this tournament.
...
Since decks were not fixed, the overall matchup table would be enormous.
...
Decks were not fixed for this tournament, so this section doesn't apply. It'll be back next tournament.
  ...
  .(round, round_match_number,
    ...
    `P1 win probability` = mapply(
      function(p1, p2, d1, d2) {
        player_effects1 <- vs_results$tidy_results$player[, p1]
        player_effects2 <- vs_results$tidy_results$player[, p2]
        deck_effects <- apply(
          vs_results$vs_array[
            ,
            as.character(codexdata::components(d1, starters)),
            as.character(codexdata::components(d2, starters))
          ],
          1,
          sum
        )
        mean(plogis(player_effects1 - player_effects2 + deck_effects))
      },
      player1,
      player2,
      deck1,
...
if (nrow(current_match_predictions) > 0) {
  observed_overall_freq <- current_match_predictions[, mean(victor == "P1")]
  freq_by_pred <- current_match_predictions[
    ...
    by = .(pred_group = round(`P1 win probability`, 0.05))
  ]
  ...
  current_score <- current_match_predictions[
    ...
    .(
      ...
      uncertainty = observed_overall_freq*(1 - observed_overall_freq),
      resolution = c(
        0,
...
Matchup evaluation not available for this tournament, back next time...
