...
DT_fill(finished_current_matches[, c("round",
                                     "player1", "player2",
                                     "deck1", "deck2",
                                     "victor", "victory")],
        dom_filter = TRUE)
...
current_matchups_table <- as.data.table(current_matchups, keep.rownames = "P1")[, c(list(P1 = factor(P1, unique(P1))), .SD),
...
formatRound(DT_fill(current_lognormal_eval[order(-matchup)], dom_filter = TRUE), 3, 3)
```
